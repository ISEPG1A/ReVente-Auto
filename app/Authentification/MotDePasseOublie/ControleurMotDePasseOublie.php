...
class ControleurMotDePasseOublie {
    ...
    private function forgot() {
        ...
        try {
            ...
            Utilitaires::envoyerJSON(['error' => 'Erreur lors de l\'envoi de l\'email.'], 500);
        }
    }
    ...
    private function reset() {
        ...
        // Validation du token
        if (!$token) {
            Utilitaires::envoyerJSON(['error' => 'Token manquant.'], 422);
        }
        
        // Validation de la longueur minimale
        if (strlen($motDePasse) < 8) {
            Utilitaires::envoyerJSON(['error' => 'Le mot de passe doit contenir au moins 8 caractères.'], 422);
        }
        ...
        if (!preg_match('/[a-z]/', $motDePasse) || !preg_match('/[A-Z]/', $motDePasse) || !preg_match('/\d/', $motDePasse)) {
            Utilitaires::envoyerJSON(['error' => 'Le mot de passe doit contenir des majuscules, des minuscules et des chiffres.'], 422);
        }
        
        // Vérifier que les deux mots de passe correspondent
        if ($motDePasse !== $motDePasseConfirm) {
            Utilitaires::envoyerJSON(['error' => 'Les mots de passe ne correspondent pas.'], 422);
        }
        ...
        if (password_verify($motDePasse, $utilisateur['password_hash'])) {
            Utilitaires::envoyerJSON(['error' => 'Le nouveau mot de passe doit être différent de l\'ancien.'], 422);
        }
